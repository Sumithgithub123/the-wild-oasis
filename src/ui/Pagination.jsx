/* eslint-disable react/prop-types */
import { HiChevronLeft, HiChevronRight } from "react-icons/hi2";
import { useSearchParams } from "react-router-dom";
import styled from "styled-components";
import { PAGE_SIZE } from "../utils/constants";

const StyledPagination = styled.div`
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: space-between;
`;

const P = styled.p`
  font-size: 1.4rem;
  margin-left: 0.8rem;

  & span {
    font-weight: 600;
  }
`;

const Buttons = styled.div`
  display: flex;
  gap: 0.6rem;
`;

const PaginationButton = styled.button`
  background-color: ${(props) =>
    props.active ? " var(--color-brand-600)" : "var(--color-grey-50)"};
  color: ${(props) => (props.active ? " var(--color-brand-50)" : "inherit")};
  border: none;
  border-radius: var(--border-radius-sm);
  font-weight: 500;
  font-size: 1.4rem;

  display: flex;
  align-items: center;
  justify-content: center;
  gap: 0.4rem;
  padding: 0.6rem 1.2rem;
  transition: all 0.3s;

  &:has(span:last-child) {
    padding-left: 0.4rem;
  }

  &:has(span:first-child) {
    padding-right: 0.4rem;
  }

  & svg {
    height: 1.8rem;
    width: 1.8rem;
  }

  &:hover:not(:disabled) {
    background-color: var(--color-brand-600);
    color: var(--color-brand-50);
  }
`;

function Pagination({ length }) {
  const [search, setsearch] = useSearchParams();
  const currpage = !search.get("page") ? 1 : Number(search.get("page"));

  const pagecount = Math.ceil(length / PAGE_SIZE);

  function next() {
    const next = currpage === pagecount ? currpage : currpage + 1;

    search.set("page", next);
    setsearch(search);
  }

  function prev() {
    const prev = currpage === 1 ? currpage : currpage - 1;

    search.set("page", prev);
    setsearch(search);
  }

  if (pagecount <= 1) return null;

  return (
    <StyledPagination>
      <P>
        Showing <span>{(currpage - 1) * PAGE_SIZE + 1}</span> to{" "}
        <span>{currpage === pagecount ? length : currpage * PAGE_SIZE}</span> of{" "}
        <span>{length}</span> results
      </P>
      <Buttons>
        <PaginationButton disabled={currpage == 1} onClick={prev}>
          <HiChevronLeft /> <span>Previous</span>
        </PaginationButton>
        <PaginationButton disabled={currpage === pagecount} onClick={next}>
          <span>Next</span> <HiChevronRight />
        </PaginationButton>
      </Buttons>
    </StyledPagination>
  );
}

export default Pagination;
